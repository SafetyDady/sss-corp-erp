...
export default function POFormModal({ open, onClose, onSuccess }) {
  ...
  const onFinish = async (values) => {
    ...
    setLoading(true);
    try {
      await api.post('/api/purchasing/po', {
        ...values,
        order_date: values.order_date?.format('YYYY-MM-DD'),
        expected_date: values.expected_date?.format('YYYY-MM-DD'),
        lines: lines.map(({ product_id, quantity, unit_cost }) => ({ product_id, quantity, unit_cost })),
      });
  ...
  const addLine = () => setLines([...lines, { key: Date.now(), product_id: undefined, quantity: 1, unit_cost: 0 }]);
  ...
  const lineColumns = [
    ...
    },
    {
      ...
      render: (v, record) => (
        <InputNumber min={1} value={v} onChange={(val) => updateLine(record.key, 'quantity', val)} style={{ width: '100%' }} />
      ),
    },
    {
      title: '\u0E23\u0E32\u0E04\u0E32/\u0E2B\u0E19\u0E48\u0E27\u0E22', dataIndex: 'unit_cost', width: 120,
      render: (v, record) => (
        <InputNumber min={0} step={0.01} value={v} onChange={(val) => updateLine(record.key, 'unit_cost', val)} style={{ width: '100%' }} />
      ),
    },
    {
      title: '', width: 50,
      render: (_, record) => (
        <Button type="text" size="small" danger icon={<Trash2 size={14} />} onClick={() => removeLine(record.key)} />
      ),
    },
  ];
    ...
    >
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Space style={{ width: '100%' }} size={16}>
          ...
          <Form.Item name="expected_date" label={'\u0E27\u0E31\u0E19\u0E17\u0E35\u0E48\u0E04\u0E32\u0E14\u0E23\u0E31\u0E1A'}>
            <DatePicker style={{ width: 200 }} />
          </Form.Item>
        </Space>
