...
> **Status Update 2026-03-05:** อัปเดตสถานะ features ที่ implement เสร็จแล้ว — C1 Invoice/AR ✅, C2 DO ✅, C3 AP ✅, C5 WHT ✅, C8 Self-service ✅, C9 Internal Recharge ✅, Go-Live Gate G1-G7 ✅, SO Flow Upgrade ✅ (รวม **161 permissions, 136 BRs**)
...
### Module 6: Sales (ขาย) — ✅ Full Cycle (updated 2026-03-05)

~~ระบบขายมี SO (Sales Order) CRUD + approve เท่านั้น ยังไม่มี flow หลัง approve~~ **แก้ไขแล้ว:**
- ✅ **SO Flow Upgrade** — DRAFT→SUBMITTED→APPROVED (+CANCELLED, reject→DRAFT), line replacement, auto-approve bypass
- ✅ **Delivery Order (C2/C3)** — DO จาก SO APPROVED, partial delivery (1 SO = multiple DOs), ship auto-cuts stock (ISSUE movements), print ใบส่งของ
- ✅ **AR Invoice** — Customer Invoice จาก SO/DO, DRAFT→PENDING→APPROVED→PAID, partial billing, payment tracking, print ใบแจ้งหนี้
- **Full Revenue Cycle:** SO → DO (ship) → AR Invoice → Receive Payment → PAID
...
**UX ที่ยังต้องทำ:**
- ~~SO ที่ approved แล้วไม่มี next step — เป็น dead-end~~ ✅ แก้แล้ว — มี DO + AR Invoice
- ~~ไม่มี Invoice generation จาก SO~~ ✅ แก้แล้ว — AR Invoice (C2)
- ~~ไม่มี Delivery Order / ใบส่งของ~~ ✅ แก้แล้ว — DO (C3)
- ~~ไม่มี Payment tracking~~ ✅ แก้แล้ว — AR Payment (C2)
- ยังไม่มี Quotation Module (C12 — planned)
...
### Module 11: Finance (การเงิน) — ✅ Core Complete (updated 2026-03-05)

~~มีเพียง summary report + CSV export เท่านั้น~~ **แก้ไขแล้ว — เพิ่ม 4 sub-modules:**
- ✅ **Supplier Invoice / AP (C1/C3)** — SupplierInvoice จาก PO RECEIVED, DRAFT→PENDING→APPROVED→PAID, WHT deducted at payment, partial billing
- ✅ **Customer Invoice / AR (C1/C2)** — CustomerInvoice จาก SO APPROVED, DRAFT→PENDING→APPROVED→PAID, no WHT, partial billing, print
- ✅ **Internal Recharge (C9)** — Fixed Budget monthly allocation per dept by headcount, DRAFT→ACTIVE→CLOSED, auto-generate entries
- ✅ **WHT/Tax (C5.2)** — OrgTaxConfig (VAT 7%, WHT 3% default), auto-calc on PO, TaxConfigTab in Admin
- **FinancePage** มี 4 tabs: Reports + AP (Supplier Invoice) + AR (Customer Invoice) + Internal Recharge

ระบบช่วยส่งข้อมูลให้บัญชีได้ แต่ **ไม่ใช่ Module ทางบัญชีตามกฎหมาย** 🆕
...
**UX ที่ยังต้องทำ:**
- ~~หน้าจอแทบว่างเปล่า — แค่ตัวเลขรวม~~ ✅ แก้แล้ว — มี AP/AR/Recharge tabs + stat cards
- ไม่มี chart หรือ graph (Phase 8 Dashboard planned)
...
| # | Feature | ปัญหาปัจจุบัน | ผลกระทบ | สถานะ |
|---|---------|--------------|---------|--------|
| C1 | **Invoice / Billing** | SO approved แล้วไม่มี Invoice | สูง | ✅ DONE (AR Invoice) |
| C2 | **Delivery Order** | SO approved แล้วไม่มีใบส่งของ | สูง | ✅ DONE |
| C3 | **Accounts Payable** | PO รับของแล้วไม่มี track การจ่ายเงิน | กลาง | ✅ DONE (AP Invoice) |
| C4 | **Budget Management** | ไม่มี budget allocation per CC | กลาง | ⬜ Planned |
| C5 | **Tax Calculation (VAT 7%)** | ไม่มีการคำนวณ VAT | สูง | ✅ DONE (WHT on PO) |
| C6 | **Multi-currency** | รองรับเฉพาะ THB | ต่ำ | ⬜ Planned |
| C7 | **WO Gantt Chart** | ไม่มี visual timeline → **ทำระดับง่ายก่อน** 🆕 | กลาง | ⬜ Planned |
| C8 | **Employee Self-service** | พนักงานดู payslip/แก้ข้อมูลไม่ได้ | กลาง | ✅ DONE (G7) |
| C9 | **Internal Recharge** 🆕 | ไม่มี cost allocation ข้ามแผนก (2 ประเภท) | สูง | ✅ DONE (Fixed type) |
| C10 | **Maintenance Module** 🆕 | ไม่มีระบบ maintenance scheduling | กลาง | ⬜ Planned |
| C11 | **Multi-Company** 🆕 | ไม่รองรับหลายนิติบุคคล | ต่ำ (อนาคต) | ⬜ Planned |
| C12 | **Quotation Module** 🆕 | ไม่มีระบบใบเสนอราคา | กลาง | ⬜ Planned |
| C13 | **Fixed Asset / Depreciation** 🆕 | ไม่มีระบบสินทรัพย์ถาวร + ค่าเสื่อม | กลาง | ⬜ Planned |

---

## C1. Invoice / Billing (ใบแจ้งหนี้) — ✅ DONE (2026-03-05)

~~**ปัญหา:** SO ที่ approved แล้ว ไม่มีทางสร้าง Invoice~~ **แก้แล้ว:**

| Feature | รายละเอียด | สถานะ |
|---------|-----------|--------|
| Invoice จาก SO | CustomerInvoice (AR) จาก SO APPROVED + optional DO | ✅ Done |
| Invoice Status | DRAFT → PENDING → APPROVED → PAID (+CANCELLED) | ✅ Done |
| Payment Tracking | CustomerInvoicePayment — partial/full, auto-PAID | ✅ Done |
| Accounts Receivable | AR tab ใน FinancePage — stat cards + overdue tracking | ✅ Done |
| Print | ARInvoicePrintView — company header + amounts + signatures | ✅ Done |

**Implementation:** C2 Customer Invoice AR — 6 permissions (finance.ar.*), 10 API endpoints, migration h8i9j0k1l2m3

---

## C2. Delivery Order (ใบส่งของ) — ✅ DONE (2026-03-05)

~~**ปัญหา:** SO ที่ approved แล้ว ไม่มีทางสร้างใบส่งของ~~ **แก้แล้ว:**

| Feature | รายละเอียด | สถานะ |
|---------|-----------|--------|
| DO จาก SO | DeliveryOrder จาก SO APPROVED + partial delivery (1 SO = multiple DOs) | ✅ Done |
| DO Status | DRAFT → SHIPPED (+CANCELLED) | ✅ Done |
| Stock Deduction | Ship creates ISSUE movement per line (auto ตัด stock, reuses create_movement) | ✅ Done |
| Print | DOPrintView — ใบส่งของ + company header + signatures | ✅ Done |
| Location Picker | Per-line warehouse/location picker on ship | ✅ Done |

**Implementation:** C3 Delivery Order — 6 permissions (sales.delivery.*), 8 API endpoints, migration j0k1l2m3n4o5, BR#129-136

---

## C3. Accounts Payable (เจ้าหนี้การค้า) — ✅ DONE (2026-03-05)

~~**ปัญหา:** PO ที่รับของแล้ว ไม่มีที่ track ว่าจ่ายเงินแล้วหรือยัง~~ **แก้แล้ว:**

| Feature | รายละเอียด | สถานะ |
|---------|-----------|--------|
| Payment Record | SupplierInvoice จาก PO RECEIVED + InvoicePayment | ✅ Done |
| AP Aging | AP tab ใน FinancePage — stat cards + overdue filter | ✅ Done |
| Payment Status | DRAFT → PENDING → APPROVED → PAID (+CANCELLED), WHT deducted at payment | ✅ Done |
| Partial Billing | multiple invoices per PO, SUM ≤ PO net_payment | ✅ Done |

**Implementation:** C1 Supplier Invoice AP — 6 permissions (finance.invoice.*), 10 API endpoints, migration g7h8i9j0k1l2, BR#113-120
...
## C5. Tax Calculation (ภาษี) — ✅ Partial DONE (WHT on PO, 2026-03-05)

| Feature | รายละเอียด | สถานะ |
|---------|-----------|--------|
| VAT 7% | OrgTaxConfig default_vat_rate (7.0%), auto-calc on PO | ✅ Done (PO) |
| Tax Report | รายงานภาษีซื้อ-ภาษีขาย | ⬜ Planned |
| Withholding Tax | OrgTaxConfig default_wht_rate (3.0%), WHT deducted on AP payment | ✅ Done |
| Tax Config | TaxConfigTab in AdminPage — admin can adjust rates | ✅ Done |

**Implementation:** C5.2 WHT on PO — OrgTaxConfig model, PO+POLine tax fields, migration f6g7h8i9j0k1, BR#99-112
...
## C8. Employee Self-service 🆕 — ✅ DONE (G7, 2026-03-05)

**Owner Decision:** ยืนยันแล้ว — พนักงานต้องทำได้

| Feature | รายละเอียด | สถานะ |
|---------|-----------|--------|
| View Payslip | MyPayslipTab — ดู payslip (RELEASED only) + print | ✅ Done (G7) |
| Edit Profile | ProfileEditModal — แก้ full_name + position via PUT /api/hr/employees/me | ✅ Done (G7) |
| Change Password | เปลี่ยนรหัสผ่านเอง | ⬜ Planned |

**Implementation:** Go-Live G7 — PayrollSlip model, /api/hr/employees/me endpoint, migration b2c3d4e5f6g7

---

## C9. Internal Recharge (Cost Allocation) 🆕 — ✅ Fixed Type DONE (2026-03-05)
...
| Feature | รายละเอียด | สถานะ |
|---------|-----------|--------|
| **Type 1: Actual Recharge** | = Job Costing ที่มีอยู่ — track ต้นทุนจริง per movement/WO | ✅ Existing |
| **Type 2: Fixed Recharge** 🆕 | Budget Planning based — annual budget per CC → monthly allocation | ✅ Done |
| Budget Setup | FixedRechargeBudget: annual_budget per CC, DRAFT→ACTIVE→CLOSED | ✅ Done |
| Allocation Driver | Headcount-based allocation per department (skip source CC) | ✅ Done |
| Auto-generate | generate_monthly_entries — auto-create per dept with rounding adjustment | ✅ Done |
| Department Cost Report | Cost Center Summary API + InternalRechargeTab in FinancePage | ✅ Done |
| Internal Invoice (optional) | สร้างเอกสาร recharge ภายใน (ไม่ใช่ tax invoice) | ⬜ Planned |
| Dashboard Integration | รวมใน Management Accounting dashboard | ⬜ Planned |
| **ผู้รับผิดชอบ** 🆕 | แผนก Admin/บัญชี เป็นผู้ดูแล Internal Recharge | — |

**Implementation:** C9 Internal Recharge — 6 permissions (finance.recharge.*), 10 API endpoints, migration d4e5f6g7h8i9, BR#89-98
...
| ลำดับ | Feature | เหตุผล | สถานะ |
|-------|---------|--------|--------|
| 1 | **🔒 Go-Live Gate (D1)** | 7 features + Engineering fixes | ✅ DONE |
| 2 | **Export & Print (B3)** | ผู้ใช้ต้องการ print PO/SO/Payslip ทุกวัน — เป็น daily need | ✅ Partial (PO/WO/Payslip/DO/AR print done) |
| 3 | **Cross-cutting UX Fix (A5)** | แก้ Select limit:500, error handling, **Role Management redesign** 🆕 | ⬜ Pending |
| 4 | **Internal Recharge (C9)** | Owner ให้ความสำคัญกับ cost visibility ระหว่างแผนก | ✅ DONE (Fixed type) |
| 5 | **Tax Calculation (C5)** | VAT 7% จำเป็นก่อน production | ✅ DONE (WHT on PO) |
| 6 | **Dashboard & Analytics (B1)** | มีข้อมูลครบแล้ว ยังไม่มี visualization | ⬜ Pending |
| 7 | **Notification Center (B2)** | ลดการ "เข้าไปดู" ว่ามีอะไรรอ | ⬜ Pending |
| 8 | **Invoice/Billing (C1) + Delivery Order (C2)** | ปิด revenue cycle (SO → DO → Invoice → Payment) | ✅ DONE (AP + AR + DO) |
| 9 | **Quotation Module (C12)** 🆕 | Owner ยืนยันต้องมี — ไม่เร่งด่วนแต่ต้องมี | ⬜ Pending |
| 10 | **Maintenance Module (C10)** | Owner ยืนยันว่าต้องมี | ⬜ Pending |
| 11 | **Inventory Enhancement (B4)** | SN sub-table, Bin, Multi-site, Consignment 🆕 | ✅ Partial (Bin done in G2) |
| 12 | **Fixed Asset / Depreciation (C13)** 🆕 | สินทรัพย์ถาวร + ค่าเสื่อมตามกฎหมาย | ⬜ Pending |
| 13 | **Mobile Responsive (B5)** | Staff ต้องกรอก Daily Report จากหน้างาน | ⬜ Pending |
| 14 | **Security (B6)** | สำคัญก่อน production scale | ⬜ Pending |
| 15 | **Multi-Company (C11)** | อนาคต — ต้องมีผู้เชี่ยวชาญ | ⬜ Pending |
| 16 | **AI Performance (B7)** | Nice-to-have, ทำทีหลังได้ | ⬜ Pending |

---

## D1 Detail: Go-Live Gate 🆕 — ✅ ALL 7 COMPLETE (2026-03-05)

> ~~**Scope Freeze:** ห้ามเพิ่ม feature อื่นจนกว่า 7 features + engineering fixes ด้านล่างจะเสร็จ~~ **ปลด scope freeze แล้ว — ทำ Post Go-Live features ได้**
> **Owner Sign-off:** ห้าม deploy production จนกว่า P'Hot จะ approve ทุก gate item

### 7 Go-Live Features

| # | Feature | รายละเอียด | Exit Criteria | สถานะ |
|---|---------|-----------|---------------|--------|
| G1 | **SPAREPART** | product_type enum เพิ่มค่า + min_stock logic | สร้าง/ค้นหา SPAREPART product ได้ + low stock alert ทำงาน | ✅ Done |
| G2 | **Bin (3-level warehouse)** | เพิ่ม `bins` table (Warehouse → Location → Bin) | RECEIVE/ISSUE/CONSUME ระบุ Bin ได้ + stock_by_bin ถูกต้อง | ✅ Done |
| G3 | **Direct PO Cost** | GR mode 2 (Direct GR) + cost เข้า WO/CC | PO charge WO ได้ + WO Cost Summary แสดง 5 components | ✅ Done |
| G4 | **Sourcer Tracking** | `PR.sourcer_id` FK → User | ดูได้ว่าใครจัดหา + report per sourcer | ✅ Done |
| G5 | **GR 2 Modes** | Stock GR vs Direct GR (ดู spec ด้านล่าง) | ทั้ง 2 mode ทำงาน + audit ครบ + reversal ได้ | ✅ Done |
| G6 | **Dept Menu Template** | Admin กำหนด menu template per department | แต่ละแผนกเห็น menu ต่างกัน + permission filter ยังทำงาน | ✅ Done |
| G7 | **Self-service MVP** | Payslip view + Profile edit | พนักงานดู payslip + แก้เบอร์โทร/ที่อยู่ได้ | ✅ Done |

**Migrations:** f8a9b0c1d2e3 (schema) + a1b2c3d4e5f7 (hardening) + b2c3d4e5f6g7 (G6+G7)
